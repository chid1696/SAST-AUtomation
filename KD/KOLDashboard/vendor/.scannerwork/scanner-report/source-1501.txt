...
var rxds = (function (exports) {
  ...
  function toggleSelection$3(group,obj,db,data,highlight,param) {
    ...
    if(param.targetElement.match("markerCluster|marker")) {
        if(param.targetElement == "markerCluster") {
            ...
            if (obj.secondary_filters.indexOf(group) === -1)  obj.filters.push(String(group)); 
            else  obj.secondary_filters.splice(obj.filters.indexOf(String(group)),1);
        }
    } else {
  ...
  function export_post$1(file_type, obj, full_data, params) {
         //Create an hidden form
                //alert( 'Not yet implemented' );
      var url, titles;
  ...
  */
  ...
  function get_parameters(d, r, e) {
      var param={};
      d.forEach(i => {param[rxds.page_controls[i].parameter] = get$8(i);});
      if (r) r.forEach(i => {if (get$8(i)) param[rxds.page_controls[i].config.parameter] = get$8(i);});
      if (e) $.extend(param, e);
      return param;
  }
  ...
  function get_param$5(p) {
      ...
      if (k) return get$8(k); else return ""; 
  }
  function find(p) {
  ...
  function reset_filter$5(k,reset_type) {
    var obj=rxds.page_controls[k];
  ...
  async function refresh$8(k, obj, data,param) {
  ...
  async function load$b(k,e) {
    ...
    await refresh$8(k, obj, data,param);
    if (dom_obj.length>0) { 
  ...
  function add_secondary_dropdown(obj, param) {
      if(obj === undefined || obj.config === undefined || param.parameter === undefined || param.lovs === undefined) {
          ...
          console.log("Optional: label, default, append_to, show_label, label_font_weight");
          console.log("Example Object: ", dropdownConfig);
          return;
      }
  ...
  manager.get_param = get_param$5;
  manager.fetchCore = fetchCore;
  manager.resize = resize$2;
  manager.util = util;
  manager.reset_filter = reset_filter$5;
