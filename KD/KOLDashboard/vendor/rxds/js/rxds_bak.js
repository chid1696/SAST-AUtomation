...
function secure_page() {
    ...
    if (authRoles && authRoles  != "" && !authRoles.match(/ALL/) ) {
        const userRoles = rxds.user.userAppRoles.split(/:/)
...
export function load_complete() {
  ...
  if (rxds.page_init) {
    ...
    $('.ui.form').form({
       ...
       on: 'blur',
        onSuccess: function() {
            //$('[name="taMessage"]').val("validated" );
            $('[name="taMessage"]').val(JSON.stringify($('.ui.form').form('get values')[1]));
            $(".ui.segment.RepReg").trigger("refresh");
        }
    }).submit( function( e ) {
        e.preventDefault();
