/**
   * This is page specific power point download function for Patient Services Dashboaed -> Patient Start Form Journey Page
   ...
   * Why this page Specific PPT Function 
   *    1. To include the start form journey chart (SVG) in the ppt slide
**/
...
function generate_ppt(include_data) {
        ...
        .forEach((v) => {
            ...
            $("#" + v + " .ui.bottom.attached.tab.segment")
                ...
                .each(function () {
                    ...
                    if (
                        rxds.page_controls[currTabArea] &&
                        rxds.page_controls[currTabArea].control_type ===
                            "Report"
                    ) {
                        window.dispatchEvent(new Event("resize"));
                        rxds.page_controls[currTabArea].table.columns
                            .adjust()
                            .draw();
                    }
                });
    ...
    function createPPT() {
        ...
        if (
            pc[arrListOfRegs[0]].region &&
            pc[arrListOfRegs[0]].region.region_seq
        ) {
        ...
        arrListOfRegs.forEach((v) => {
            ...
            try {
                ...
                if (pc[v].control_type === "Chart G2") {
                    ...
                    pc[v].control_type === "Plugin" &&
                    pc[v].config.has_svg === "Y"
                ) {
                    title = pc[v].config.title;
                    img = $("#" + v).find("a.download-svg")[0].href;
                    imgX = 0.1;
                    imgWidth = 11.04;
                    imgHeight = 5.25;
                ...
                console.log(
                    "\n\nError!!! \n Something went wrong while processing this region: " +
                        pc[v].config.title +
                        " (" +
                        v +
                        ")"
        ...
        Object.keys(pc)
            ...
            .forEach((v) => {
                $(
                    "#" + v + " .ui.bottom.attached.tab.segment.active"
                ).removeClass("active");
    ...
    var htm2CanvasArr = Object.keys(pc).filter(
        (v) =>
            (pc[v].control_type === "Report" &&
                pc[v].config.include_in_ppt === "Y") ||
            (pc[v].control_type === "Report" &&
    ...
    if (htm2CanvasArr.length === 0) {
        ...
        htm2CanvasArr.forEach((v, itm, arrr) => {
            html2canvas(document.querySelector("[id='" + v + "']")).then(
                (canvas) => {
